...
PreComposeLayer* createVideoLayer(movie::VideoTrack* track, const movie::MovieSpec& spec) {
    ...
    if (track->content.effect == "anti-duplicate") {
      ...
      //朦胧雾化（全局，类似“眼睛没揉开”的朦胧 + 发白）
      // auto haze = new pag::EllipseHazeEffect();
      // float blurRadius = 2.0f;  //（边界软化半径，像素）
      // float bloom = 0.6f;   //（泛光/朦胧强度，0~1）
      // float whiten = 0.45f; //（发白/白雾强度，0~1）
      // float opacity = 1.0f;
      ...
      // opacity = std::max(0.0f, std::min(1.0f, opacity));
      // haze->blurRadius = new pag::Property<float>(blurRadius);
      // haze->bloom = new pag::Property<float>(bloom);
      // haze->whiten = new pag::Property<float>(whiten);
      // haze->effectOpacity = new pag::Property<pag::Opacity>(
      //     static_cast<pag::Opacity>(std::max(0, std::min(255, static_cast<int>(opacity * 255.0f)))));
      // vidPreComposeLayer->effects.push_back(haze);

      //白纱
      // float strength = 0.6f;   //白纱“厚度/发白程度”
      // float opacity = 0.60f;    //混合透明度（最终叠加强度）
      // bool uniform = true;    //色块均匀
...
movie::ArticleParagraph generateParagraph(const std::string& fontFamilyName, const std::string& text, int fontSize, 
  ...
  // std::cout << "generateParagraph, get one, text:" << p.text << ", height:" << p.heightInP << ", lines:" << lines << ", font size:" << fontSize
  //   << ", leading:" << leading << ", tracking:" << tracking << ", box width:" << boxWidth << std::endl;
  return p;
}
...
std::shared_ptr<JSONComposition> JSONComposition::Load(const std::string& json_str, std::string tmp_dir, const std::function<void(int)>& progressCB) {
    ...
    std::string tmpDir;
    if (tmp_dir.empty()) {
      fs::path temp_dir = create_temp_directory();
      tmpDir = temp_dir.string();
      printf("system tmp dir: %s\n", tmpDir.c_str());
    } else {
      tmpDir = tmp_dir;
